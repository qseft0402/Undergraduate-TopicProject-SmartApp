...
//        else
        text_Oxygen_min.append(getYMin(Oxygen_datas)+"");
...
//        else
        ...
        text_pulse_min.append(getYMin(pulse_datas)+"");
...
//        DataIntoIf1(cursor[2],power_datas,false);
    ...
    int getYMin(DataPoint[] dataPoints){
        int min=0;
        for (int i=0;i<dataPoints.length;i++){//先做一遍找出第一個不為0的 找出一個可比較值
            if((int)dataPoints[i].getY() != 0) {
                min = (int) dataPoints[i].getY();
                Log.d("getMin1", "min= " + min);
                break;
            }
        }
        for (int i=0;i<dataPoints.length;i++){
            if((int)dataPoints[i].getY() < min && (int)dataPoints[i].getY()!=0)
                min=(int)dataPoints[i].getY();
            Log.d("getMin",""+dataPoints[i].getY());
        }
        Log.d("getMin","return "+min);
        return min;
    }
